// Chuỗi thời gian của Bộ Nhớ Ký Ức: cung cấp danh tính nguồn ổn định, kiểm tra hash và ẩn an toàn cho nội dung chính, bản tóm tắt và cập nhật thực thể.
window.MEMORY_ENGINE_TIMELINE = (function() {
  ...
  // Đồng bộ, ổn định và đủ rẻ; hash chỉ dùng để phát hiện thay đổi, không nhằm mục đích bảo mật.
  ...
  function scheduleSave() {
    ...
    saveTimer = setTimeout(() => {
      ...
      } catch (_) { /* ID nguồn đã có hiệu lực trong bộ nhớ; lưu thất bại không chặn luồng xử lý trí nhớ. */ }
    }, 250);
  }
  ...
  function auditRefs(refs) {
    ...
    // Các node kế thừa từ cuộc trò chuyện khác thuộc lịch sử đã lưu trữ trước Root, không đối chiếu lại với tầng của cuộc trò chuyện hiện tại.
  ...
  function refsToConversation(refs) {
    ...
    return audit.refs.map(ref => {
      ...
      return `[Tầng ${found.layer}]【${clean(message?.name) || fallback}】\n${clean(message?.mes)}`;
    }).filter(Boolean).join('\n\n');
  }
  ...
  async function syncHidden(coveredMessageIds) {
    ...
    if (typeof exec === 'function') {
      ...
      } catch (_) { /* Trạng thái trong bộ nhớ đã được đồng bộ. */ }
    }
    scheduleSave();
