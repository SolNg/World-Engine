// memory-engine-data.js — Trạng thái và điểm lưu của Bộ Nhớ Ký Ức, tách biệt theo từng cuộc trò chuyện
window.MEMORY_ENGINE_DATA = (function() {
  ...
  function normalizeState(state) {
    ...
    // Tương thích với cấu trúc world_memory/item từng dùng trong giai đoạn phát triển; cấu trúc ổn định công khai chỉ giữ lại entity_memory/object.
    ...
    next.event_memory.small_summaries = next.event_memory.small_summaries.map((item, index) => {
      ...
      // Nguồn liên động chỉ dùng để nhận diện reroll trong cùng tầng; bản tóm tắt thông thường không mang các trường này.
    ...
    // Ở bản 0.4.x chỉ có một big_summary duy nhất được cuộn tiếp; sau khi nâng cấp sẽ chuyển thành big_summaries có thể nối thêm.
    ...
    if (!next.timeline || typeof next.timeline !== 'object' || Array.isArray(next.timeline)) {
      // Dữ liệu cũ không có chuỗi sự kiện: đóng gói toàn bộ trạng thái cuối cùng lúc đó thành Root, các cập nhật sau này mới đi vào chuỗi node.
  ...
  // Tách biệt với checkpoint “trích xuất lại” thông thường: ở đây chỉ lưu nền trí nhớ trước lần ghi đầu tiên của tầng liên động thế giới hiện tại.
  ...
  function rebaseImportedState(state) {
    ...
    // Nhập dữ liệu là nối trí nhớ hiện có vào cuộc trò chuyện hiện tại; con trỏ tầng phải bắt đầu lại từ cuộc trò chuyện hiện tại.
    next.chatLayer = layer;
    next.event_memory.small_summary_layer = layer;
    // Khi nhập dữ liệu giữa các cuộc trò chuyện khác nhau, nén chuỗi có thể phát lại từ cuộc trò chuyện cũ thành Root của cuộc trò chuyện đích.
    // Nhờ vậy cuộc trò chuyện mới chỉ xử lý các tầng mới về sau, không nhầm lẫn tầng có cùng số thứ tự với node nguồn cũ.
  ...
  function importData(payload) {
    ...
    if (!state || typeof state !== 'object' || Array.isArray(state)
      ...
      throw new Error('Thiếu dữ liệu trí nhớ hợp lệ');
    }
